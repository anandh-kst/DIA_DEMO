...
app.use("/onesify/network/docs", swaggerUi.serve, swaggerUi.setup(swaggerJsDoc));
...
app.get("/onesify/network/common/get_order_number", commonJs.get_order_number);
// app.get("/onesify/network/common/modifyIrDate", commonJs.modifyIrDate);
...
server.listen(port, () => {
  logger.info(`One Sify Network ILL API is running on port ${port} at ${currentDate}`);
  console.log(`One Sify Network ILL API is running on port ${port} at ${currentDate}`);
});
