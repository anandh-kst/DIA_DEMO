...
exports.send_mail_to_all = async (req, res, next) => {
  try {
    ...
    const poPdfUrl = `${process.env.APP_PATH}/onesify/network/docu_sign/${
      quote.quoteType === "New"
        ? "get_sign_order"
        : "get_modify_sign_order/true"
    }/ILL-SO-${reqId}`;
    ...
    const sendMail = await common.send_mail(
      mailIds,
      null,
      subject,
      html,
...
exports.send_mail_to_sign = async (req, res, next) => {
  try {
    ...
    const companyRecord = await loginDB
    ...
    if (!companyName) {
      throw new Error(
        `Company not found in companies table for companyId: ${quote.companyId}`
      );
    }
...
<body>
  ...
  <style>
        ...
        <div
          ...
          <table align="center" border="0" cellpadding="0" cellspacing="0" width="70%" bgcolor="white">
            <tbody>
              ...
              <tr>
                <td style="padding: 25px;">
                  <p>Dear <span style="font-size: 18px; color: #0E3346;">${
                    cc.length > 0 ? cc[0].name : "Recipient"
                  }</span>,</p>
                  <p>Greetings from Sify!</p>
                  <p><strong>${customerName}</strong>, Sify’s Channel Partner from
                    <strong>${
                      companyRecord.cpcompanyName
                    }</strong>, has shared an order document for your review.
                    Please find the order details below:</p>
                  <p><strong>Order Details</strong></p>
                  <ul style="padding-left: 20px;">
                    <li><strong>Name of document:</strong> ILL-SO-${reqId}</li>
    ...
    if (parentRole === "CP + Customer") {
      ...
      // const ccEmails = [...ccArray, "sowmiyakstinfotech@gmail.com", "kiran.sudharsan@sifycorp.com", "murali.janakiraman@sifycorp.com"];
      const ccEmails = [
        ...ccArray,
        "sowmiyakstinfotech@gmail.com",
        "esakkirajkstinfotech@gmail.com",
...
exports.test_pdf = async (req, res, next) => {
  try {
    ...
    const { reqId } = await reqID.findOneAndUpdate(
      { id: "file_id" },
      { $inc: { reqId: 1 } }
    );
  ...
  `;
    ...
    const result = await oracalDb.execute(query, bindParams, {
      autoCommit: true,
    });
...
exports.send_proposal_mail = async (req, res, next) => {
  try {
    const { reqId, version, channelPartnerName, channelPartnerEmail, to, cc } =
      req.body;
    ...
    if (!quote) {
      return res.status(404).json({
        error: `No quote found for reqId=${reqId}, version=${version}`,
      });
    }
    ...
    const toEmailName = to.map((entry) =>
...
exports.getSharedMails = async (req, res, next) => {
  try {
    ...
    const createdDate = moment(doc.createdDate).format("MMM D,YYYY [at] HH.mm");
...
exports.send_mail_toapproval = async (req, res, next) => {
  try {
    const {
      reqId,
      priceType,
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Missing required fields. Expected: reqId, priceType, lineItem{name, floorPrice, ceilPrice, askPrice, standardPrice}, customerName, channelPartnerName, channelPartnerEmail",
      });
    }

    const to = ["sugavanesh.mayavel@sifycorp.com", "vaishnavi.srinivasan@sifycorp.com"];
    const subject = `Sify - Channel Partner - ${priceType} Pricing Approval Request`;

    const html = `   
  <!DOCTYPE html>
<html>
  <body style="font-family:'Myriad Pro',sans-serif; color:#333; line-height:1.5; margin:0; padding:0; background-color:#ffffff;">
    <div style="max-width:600px; margin:20px auto; padding:20px;">
      <p>Dear Team,</p>
      <p>
        Customer <b>${customerName}</b>, via channel partner <b>${channelPartnerName}</b>, 
        has requested the following <b>${priceType}</b> price details for <b>${lineItem.name}</b>.
      </p>

      <h4 style="color:rgb(60,75,73); margin-top:30px; text-align:center;">
        ${priceType} Price Approval Request
      </h4>

      <table style="border-collapse:collapse; width:100%; margin:20px 0; font-size:14px;">
        <tr>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Request ID</th>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Line Item</th>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Floor Price</th>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Ceil Price</th>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Ask Price</th>
          <th style="border:1px solid #ddd; padding:10px; text-align:center; background-color:#f4f4f4;">Standard Price</th>
        </tr>
        <tr>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${reqId}</td>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${lineItem.name}</td>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${lineItem.floorPrice}</td>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${lineItem.ceilPrice}</td>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${lineItem.askPrice}</td>
          <td style="border:1px solid #ddd; padding:10px; text-align:center;">${lineItem.standardPrice}</td>
        </tr>
      </table>

      <div style="text-align:center;">
        <a href="" 
           style="display:inline-block; cursor:pointer; padding:10px 20px; color:#fff !important; text-decoration:none; border-radius:4px; margin:10px 10px 20px; cursor:pointer; background-color:#33d87d;">
          Approve
        </a>
        <a href="" 
           style="display:inline-block; cursor:pointer; padding:10px 20px; color:#fff !important; text-decoration:none; border-radius:4px; margin:10px 10px 20px; cursor:pointer; background-color:#e15e5e;">
          Reject
        </a>
      </div>
    ...
    const sent = await common.send_mail(to, [], subject, html, null);
