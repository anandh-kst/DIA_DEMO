...
exports.get_base_plan_details = async (req, res, next) => {
  try {
    ...
    let { reqBandwidth, valueAddedService = [], additionalPrice = [], connectionType, mastHeight, cxmConformation, mastType, opex, capex, otherIspOtc,totalKm, otherIspArc } = locationDetails;
    ...
    if (hasRateCard && rateCode.includes("CXM_RC_SPL")) {
      ...
      if (connectionType === "Wireless" && parseInt(mastHeight) !== 0 && !cxmConformation) {
        ...
        basePlan.additionalPrice.push({
          ...
          unit: hasTowerPrice ? tower[0].unit : "Meter",
          mastType: mastType,
        });
      ...
      if (connectionType === "Wireless" && parseInt(mastHeight) !== 0 && !cxmConformation) {
        ...
        const {locationDetails} = quote
        console.log("fiber mast", locationDetails);
        basePlan.additionalPrice.push({
          opex: opex,
          capex: capex,
          totalKm: totalKm
